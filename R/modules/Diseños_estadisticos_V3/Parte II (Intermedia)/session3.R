...
session3UI <- function(id) {
  ...
  tagList(
    ...
    bslib::navset_tab(
      ...
      # ===== PESTAÑA 1: MRLS (Fundamentos & datos base) =====
      bslib::nav_panel(
        title = "1) MRLS & Datos",
        tags$div(class = "mb-3",
          tags$p(
      ...
      # ===== PESTAÑA 2: Diagnóstico de supuestos =====
      bslib::nav_panel(
        title = "2) Diagnóstico",
        tags$p(
      ...
      # ===== PESTAÑA 3: No lineales & Selección (AIC y R²aj) =====
      bslib::nav_panel(
...
session3Server <- function(input, output, session) {
  ...
  read_csv_xy <- function(path, header = TRUE, sep = ",") {
    # Lectura robusta con utils::read.table (evita dependencias)
    df <- utils::read.table(path, header = header, sep = sep, dec = ".", quote = "\"'",
                            comment.char = "", stringsAsFactors = FALSE)
    # Normalizamos nombres esperados: x,y
  ...
  observeEvent(input$s3_run_diag, {
    # No hace nada: sólo fuerza dependencia de reactividad
    invisible(TRUE)
  })
  ...
  output$s3_levene_out <- renderPrint({
    ...
    # Agrupamos x en k quantiles para aproximar niveles del predictor
  ...
  # ---------------------------
  # Ajuste seguro de modelos (con tryCatch)
  ...
  s3_fit_models <- eventReactive(input$s3_run_compare, {
    ...
    # Si forzamos misma base: si incluye log, todos usan x>0
  ...
  # ---------------------------
  # PESTAÑA 4: ejercicios prácticos (reporte focal)
  # ---------------------------
  s3_fit_focal <- reactive({
    df <- df_rv(); req(nrow(df) > 3)
    exm <- input$s3_ex_model
    if (isTRUE(input$s3_ex_use_weights)) {
      # Ponderación simple 1/ŷ (demostrativa)
      # Ajuste inicial para obtener ŷ:
      ...
      mod <- switch(
        ...
        log  = stats::lm(y ~ log(x), data = dplyr::filter(df, x > 0)),
        NULL
      )
    }
    mod
