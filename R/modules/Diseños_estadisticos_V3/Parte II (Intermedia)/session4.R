...
pestanna3_session4_v3_server <- function(input, output, session, datos_rcbd) {
  ...
  output$out_aov1 <- renderPrint({ shiny::req(ajuste_aov1()); summary(ajuste_aov1()) })
  output$out_aov2 <- renderPrint({ shiny::req(ajuste_aov2()); summary(ajuste_aov2()) })
...
pestanna5_session4_v3_server <- function(input, output, session) {
  # No server logic needed
}
...
session4_v3Server <- function(input, output, session) {
  ...
  pestanna5_session4_v3_server(input, output, session)
