...
# Pestaña 4: Post-hoc (LSD correcto)
...
pestanna3_session6_v3_server <- function(input, output, session, design_obj, datos_para_analisis, strip_fit, has_pkg) {
...
pestanna4_session6_v3_server <- function(input, output, session, datos_para_analisis, strip_fit) {
  compute_LSDs <- eventReactive(input$btn_correr_posthoc, {
    shiny::req(strip_fit()); fit <- strip_fit()
    df <- datos_para_analisis(); shiny::req(df)
  ...
  output$out_lsd_A  <- renderPrint({ x <- compute_LSDs(); shiny::req(x$A);  print(x$A)  })
  output$out_lsd_B  <- renderPrint({ x <- compute_LSDs(); shiny::req(x$B);  print(x$B)  })
  output$out_lsd_AB <- renderPrint({ x <- compute_LSDs(); shiny::req(x$AB); print(x$AB) })
}
...
pestanna5_session6_v3_server <- function(input, output, session) {
  ...
  output$ex_lsd <- renderPrint({
    fit <- ex_fit(); df <- ex_data(); shiny::req(fit, df)
    df$AB <- interaction(df$col, df$row, drop = TRUE)
...
session6_v3Server <- function(input, output, session) {
  ...
  pestanna3_session6_v3_server(input, output, session, design_obj, datos_para_analisis, strip_fit, has_pkg)
  pestanna4_session6_v3_server(input, output, session, datos_para_analisis, strip_fit)
  pestanna5_session6_v3_server(input, output, session)
